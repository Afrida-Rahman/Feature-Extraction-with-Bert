...
def use_grid_ANN(use_class, model_type_list, ctl, bert_type):
    best_auc = -1
    save_param_EI = []
    save_score = []
    data = pd.read_excel('./indp_fea/new_dl_param.xlsx', index_col=False)

    for model_type in model_type_list:
        idx = data.iloc[
              list((data.iloc[:, 0] == bert_type) & (data.iloc[:, 1] == ctl) & (data.iloc[:, 2] == model_type)),:].index
        b = data.iloc[idx, 3].item()
        l = data.iloc[idx, 4].item()
        beta = data.iloc[idx, 5].item()
        e = data.iloc[idx, 6].item()
...
def use_fold_ANN(model_type_list=[],number=[1], train_dir='./', label_dir='./', ctl='CLS', bert_type='base',j=0,need=0,average='no'):
    ...
    for i in number:
        ...
        param_EI,scores = use_grid_ANN(Train_ANN,model_type_list,ctl_,bert_type2[j])

        import pandas as pd
        param_EI = pd.DataFrame(param_EI,columns=None,index=None)
        param_EI.reset_index(drop=True)

        param_EI.to_csv('./' + bert_type +'/' +ctl_ + '/best_indp_EI.csv', header=False,index=False)
        #param_EI.to_csv('./' + bert_type +'/' +ctl_ +'/' +'fold' +str(i)+  '/parm_EI.csv', header=False,index=False)

        score_dict = {}
        # for ii in range(len(model_type_list)):
        #     s_temp = pd.DataFrame(scores[ii],columns=None,index=None)
        #     s_temp.reset_index(drop=True)
        #     s_temp.to_csv('./' + bert_type +'/' +ctl_+ '/' +'fold' +str(i) + '/scores' + model_type_list[ii] + '.csv', header=False,index=False)
        #     score_dict[model_type_list[ii]] = scores[ii]
        #
        # joblib.dump(score_dict,'./' + bert_type +'/' +ctl_+ '/' +'fold' +str(i) + '/scores.np')
...
if __name__ == '__main__':
    ...
    for type_ in type_list:
        ...
        nlp_type_ = 'new_' + type_
        j_dict = {'new_bert': 0, 'new_probert': 1, 'new_tape': 2}
        j = j_dict[nlp_type_]  # 0 bert, 1 probert, 2 tape.
        number = [1]

        train_dir = '/mnt/raid5/data4/publice/gly_site_pred/' + type_ + '_fea/'
