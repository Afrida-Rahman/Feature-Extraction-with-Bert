...
def rf_xlf(md=4, nesti=1400):
    return RandomForestClassifier(max_depth=md, n_estimators=nesti, n_jobs=-1)
...
def grid_xlf(key, f_key, xlf, param, train, y_train, x_test, y_test):
    ...
    param_grid_dict = list(ParameterGrid(param))
...
def fold_grid_xlf(file_featrue, filename_label, save_file='./save_best_param/', number=0):
    ...
    # feature_dict = {'AAC': need_aac, 'PWAA': need_pwaa, 'DBPD': need_dbpb, 'EBGW': need_ebgw, 'kspace': need_kspace,
    #                 'KNN': need_knn}
    feature_dict = {'KNN': need_knn}

    for f_key in feature_dict.keys():
        save_file = './single_feature_fold_oldknn/' + f_key + '/'
        if not os.path.exists('./single_feature_fold_oldknn/'):
            os.mkdir('./single_feature_fold_oldknn/')
        ...
        for i in range(1, number + 1):
            ...
            for key in xlf_dict.keys():
                EI = grid_xlf(key, f_key, xlf_dict[key], param_grid[key], x_train, y_train, x_test, y_test)
                EI_list.extend(EI)
